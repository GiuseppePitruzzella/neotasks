...
const UserInterface: React.FC<UserInterfaceProps> = ({ backendName }) => {
    ...
    return (
        <div className={`user-interface ${bgColor} ${backendName} w-full h-full p-4 my-4 rounded-2xl shadow`}>
            ...
            <div>
                <form onSubmit={createUser} className="flex flex-col justify-center items-center space-y-4">
                    ...
                    <input 
                        ...
                        value={newUser.email}
                        onChange={(e) => setNewUser({ ...newUser, email: e.target.value, job: e.target.value })}
                        className={`${bgColor} p-4 rounded-2xl shadow-neo`}
                    />
                    <button type="submit" className={`${bgColor} p-2 w-52 rounded-2xl shadow-neo text-white hover:bg-blue-500`}>Add</button>
                </form>
            </div>
            ...
            <div className="my-8 space-y-2">
                {users.map((user) => (
                    <div key={user.id} className={`flex items-center justify-between ${bgColor} shadow-neo w-64 rounded-2xl hover:shadow-lg`}>
